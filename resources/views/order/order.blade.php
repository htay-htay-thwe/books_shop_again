...
@section('content')
    ...
    <div class="mt-0 page-container">
        <div class="shadow-sm">
            <h2 class="mb-2 text-center">Order List</h2>
            <select id="orderStatus" name="status" type='text' class="p-2 form-select col-2" aria-label="Default select example">
            ...
            <tbody id="dataList">
                  ...
                  <tr>
                    ...
                    <td name="bookImage"><img src="{{ asset('storage/'.$data->bookImage) }}" alt="" class="card-img w-25"/> {{ $data->bookName }}</td>
                    <td name="address">Address</td>
                    <td>{{ $data->created_at }}</td>
                    <td name="price">{{ $data->price }}</td>
                    <td>
                        <select  name="status" type='text' class="form-select au-input au-input--full " id="statusChange">
                        <option value="true" @if($data->selected == 'true') selected @endif>Sent</option>
                        <option value="false" @if($data->selected == 'false') selected @endif>Pending...</option>
                        <option value="cancel" @if($data->selected == 'cancel') selected @endif>Cancel</option>
                      </select>
                    </td>
                  </tr>
                  @endforeach
